 ...
 * - 회사마다 다를 수 있음, 프로젝트마다 다를 수 있음 (회사의 code convention guideline 참고)
 * 카멜(camel) : 소 - 대 - 소
 *              ex) let coffeeType, makeCoffee()
 *              함수, 변수, 메소드
 * 파스칼(pascal) : 대 - 대 - 소
 *              ex) class UserInfo{}, function Person(){}
 *              생성자, 클래스
 * lower(전체소문자) : 전체를 소문자로
 *                ex) let, const 예약어, 키워드
 * upper(전체대문자) : 전체를 대문자로 (const 대문자로 쓴거 보고 변하지 않는 상수값이구나를 알 수 있음)
 *                  ex) const PI = 3.14; const PI = Math.PI;
 */
...
// console.log("toString" in coffee); // toString이 나한테 없지만 true로 나온 이유?

// coffee 객체에는 없는 속성이나 js 최상위 객체인 Object에 있는 속성도 사용할 수 있기 때문에 toString도 true가 나온다.
// 즉, 나한테 없어도 최상위 객체 Object가 갖고 있는 속성을 내가 쓸 수 있기때문에 true!
...
// const 사용하는 방법
// 상수 값을 할당함.
// const를 썼을 때  const p = {}, p.name = " ... " -> p의 속성값은 수정 가능
// 근데 p 자체 ({ o: o })를 바꾸려하면 수정 불가능
const people = {
  userName: "손흥민",
  age: 30,
};
